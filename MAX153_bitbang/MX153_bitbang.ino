...
bool new_image = 0;
void setup() {  //MAX153 outputs bits, DX=GPIOX fo simplicity
  gpio_init(0);
  gpio_set_dir(0, GPIO_IN);  //MAX153 D0
  gpio_init(1);
  gpio_set_dir(1, GPIO_IN);  //MAX153 D1
  gpio_init(2);
  gpio_set_dir(2, GPIO_IN);  //MAX153 D2
  gpio_init(3);
  gpio_set_dir(3, GPIO_IN);  //MAX153 D3
  gpio_init(4);
  gpio_set_dir(4, GPIO_IN);  //MAX153 D4
  gpio_init(5);
  gpio_set_dir(5, GPIO_IN);  //MAX153 D5
  gpio_init(6);
  gpio_set_dir(6, GPIO_IN);  //MAX153 D6
  gpio_init(7);
  gpio_set_dir(7, GPIO_IN);  //MAX153 D7

  gpio_init(LED_WRITE);
  gpio_set_dir(LED_WRITE, GPIO_OUT);  //additional LED, you can get rid of it
  gpio_init(LED);
  gpio_set_dir(LED, GPIO_OUT);  //internal LED, flashes when data are transmitted
  gpio_init(READ_CAMERA);
  gpio_set_dir(READ_CAMERA, GPIO_IN);
  gpio_pull_down(READ_CAMERA);  //CAM_READ, signal to trigger reading from camera
  gpio_init(CLOCK);
  gpio_set_dir(CLOCK, GPIO_IN);
  gpio_pull_down(CLOCK);  //CAM_CLOCK signal to trigger reading a pixel from camera
  gpio_init(READ_MAX153);
  gpio_set_dir(READ_MAX153, GPIO_OUT);
  gpio_put(READ_MAX153, 1);         ///MAX153 RD, signal to trigger reading to the MAX153 in mode 0
...
void loop1()  //core 1 polls the camera
{
  if ((gpio_get(READ_CAMERA) == 1))  //READ signal from camera goes high on a rising front
  {
    ...
    while (counter < (128 * 120))  //last 8 lines are junk, no need to record them
    {
      ...
      CamData[counter] = gpio_get_all() ;  //get all the GPIOs state at once. The mask is not necessary as packing an int into a char does the same job...
      gpio_put(READ_MAX153, 1);            //MAX153 returns in idle mode
      counter++;
      while (gpio_get(CLOCK) == 0)
        ;  //wait until CLOCK camera goes high again
    }
    while (gpio_get(READ_CAMERA) == 1)
      ;                //wait until READ_CAMERA camera goes low again
    gpio_put(LED, 0);  //triggers the internal LED off
    if (new_image == 0) {
      memcpy(OutputData, CamData, sizeof(CamData));
      new_image = 1;
    }
  }
}
